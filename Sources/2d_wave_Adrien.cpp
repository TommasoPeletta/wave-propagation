...
//Pour lancer le code : g++ 2d_wave_Adrien.cpp -o lookAtThis -lglut -lGL
//                      ./lookAtThis
        ...
        for (int k = 0; k < q+1; k++){
          switch (k) {
            ...
            case 1 : if (j != (sizeY-1)) {
                      ...
                      f_in[i][0][k] = f_out[i][j][k]*b;}
                      break;
            case 2 : if (i != 0) {
                        f_in[i-1][j][k] = f_out[i][j][k];
                      } else {
                        f_in[sizeX-1][j][k] = f_out[i][j][k]*b;}
                      break;
            case 3 : if (j != 0) {
                      f_in[i][j-1][k] = f_out[i][j][k];
                    } else {
                      f_in[i][sizeY-1][k] = f_out[i][j][k]*b;}
                    break;
            case 4 : if (i != (sizeX-1)) {
                      f_in[i+1][j][k] = f_out[i][j][k];
                    } else {
                      f_in[0][j][k] = f_out[i][j][k]*b;}
                    break;
          }
        }
      }
